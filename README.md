...
A launch opens a second kind of break. When a fighter is knocked into tumble, the other one chases on its own: it runs underneath, then jumps and double-jumps up to meet them. Just before the chaser reaches them, the match freezes. The chaser picks a move or a combo (up air, up air into up air, neutral air into forward air, up smash to cover the landing) or waits for the read. The launched fighter picks a reaction: air dodge away, jump away, drift away, or an aerial. If still in hitstun, the reaction comes out once it ends. A reaction that is still waiting when the fighter lands is dropped. If the follow-up launches again, the chase starts over, so juggles become a series of reads. Launches offstage and trades keep playing. Offstage recovery, the ledge jump, and getup still happen on their own. The result returns to the draft.
